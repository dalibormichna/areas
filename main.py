...
def debug():
    results = {}
    # Test ruznych payloadu
    tests = [
        {"czNace": ["56"], "pocet": 2, "start": 0},
        {"czNace": ["5610"], "pocet": 2, "start": 0},
        {"obchodniJmeno": "restaurace", "pocet": 2, "start": 0},
    ]
    for payload in tests:
        key = str(payload.get("czNace", payload.get("obchodniJmeno", "?")))
        r = requests.post(ARES_SEARCH, json=payload, headers=HEADERS, timeout=15)
        results[key] = {"status": r.status_code, "body": r.text[:400]}
