...
function renderTable(){
  ...
  document.getElementById('tBody').innerHTML=sl.map(d=>'<tr onclick="openDetail(\''+d.ico+'\')">'+'<td class="td-name">'+d.name+'</td>'+'<td class="td-ico">'+d.ico+'</td>'+'<td>'+bdg(gt(d.nace))+'</td>'+'<td>'+fmt(d.date)+'</td>'+'<td class="td-addr">'+d.addr+'</td>'+'</tr>').join('');
}
...
function renderDetail(d, res){
  ...
  var h='';
  h+='<div class="dp-section"><h4>Z&#225;kladn&#237; &#250;daje</h4>';
  h+='<div class="dp-row"><span class="k">I&#268;O</span><span class="v">'+d.ico+'</span></div>';
  h+='<div class="dp-row"><span class="k">Vznik</span><span class="v">'+fmt(d.date)+'</span></div>';
  h+='</div>';
  h+='<div class="dp-section"><h4>Osoby</h4>';
  osoby.forEach(function(o){h+='<div class="person-card">'+o.jmeno+'<div class="pt">'+o.funkce+'</div></div>';});
  h+='</div>';
  h+='<a class="dp-btn" href="https://www.google.com/search?q='+encodeURIComponent(d.name+' '+d.obec)+'" target="_blank">&#8599; Google</a>';
  h+='<a class="dp-btn" style="margin-top:6px" href="https://maps.google.com/?q='+encodeURIComponent(d.name+' '+d.addr)+'" target="_blank">&#8599; Mapy</a>';
