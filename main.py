    ...
    try:
        ...
        for s in subjekty:
            ...
            addr = ", ".join(p for p in [
                (ulice + " " + cislo).strip(), str(psc) if psc else "", obec_n
            ] if p)
...
def detail(ico):
    ...
    try:
        r = requests.get(
            f"{base_url}/ekonomicke-subjekty/{ico}",
        ...
        try:
            r2 = requests.get(
                f"{base_url}/ekonomicke-subjekty-rzp/{ico}",
                headers={"Accept": "application/json", "User-Agent": "Mozilla/5.0"},
                timeout=15,
            )
            if r2.status_code == 200:
                ...
                if zaznamy:
                    ...
                    rzp = {
                        "osoby": [{"jmeno": (o.get("jmeno", "") + " " + o.get("prijmeni", "")).strip(),
                                   "funkce": o.get("typAngazma", ""),
                                   "od": o.get("platnostOd", "")} for o in osoby],
...
def test():
    try:
        r = requests.post(
            ARES,
            json={"czNace": ["5610"], "pocet": 3, "start": 0},
