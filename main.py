...
def search():
    ...
    # Zjisti klic pro NACE_MAP
    key = ",".join(sorted(nace_input))
    nace_codes = NACE_MAP.get(key) or NACE_MAP.get(nace_input[0] if nace_input else "") or nace_input
    ...
    for nace in nace_codes:
        ...
        if not obec and len(all_results) >= pocet:
            break
